...
export const login = async (req, res, next) => {
            try {
                        ...
                        if (user.lastActiveDate) {
                                    ...
                                    if (diffDays === 0) {
                                                // already logged in today — streak count unchanged, but
                                                // make sure today is actually recorded in streakDates.
                                                // This guards against streakDates being empty for today
                                                // (e.g. this login happened before streakDates existed, or
                                                // this user was backfilled) — without this check, a repeat
                                                // same-day login would just skip past it forever and
                                                // today's date would never get recorded.
